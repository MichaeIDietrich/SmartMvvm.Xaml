...
public sealed class Switch : LogicalBase
{
    ...
    private object _nullValue = AvaloniaProperty.UnsetValue;
    ...
    {
        if (_nullValue is MarkupExtension or IBinding)
            return true;

        if (Default is MarkupExtension or IBinding)
            return true;

        foreach (var entry in Cases)
        {
            if (entry.Key is MarkupExtension or IBinding)
                return true;

            if (entry.Value is MarkupExtension or IBinding)
                return true;
        }
    ...
    protected override object Evaluate(IReadOnlyList<object> values)
    {
        ...
        if (values.Count == 1)
        {
            if (key is null || ReferenceEquals(key, AvaloniaProperty.UnsetValue))
                return _nullValue;

            return Cases.TryGetValue(key, out var value) ? value : Default;
        }
